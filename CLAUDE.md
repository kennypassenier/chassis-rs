...
| Next gate | The four consumers adopt in their own sessions; CF-12 and fix-3 close on their reports. One design question is open and unanswered: a public item moved between modules reads as a removal plus an addition, so a refactor forces a major — technically true, since those long paths are nameable, but not what a consumer feels when it reaches the item through one of the seven crate-root re-exports |
