...
| Next gate | **The release of 1.9.0** — batch 4, round 5 and kp-themes 5.1.0 all sit under `[Unreleased]` (15 entries) while the crate still says 1.8.0. Kenny's moment, and rule 39 applies: his own look at something live comes first. Open behind it: `scripts/drill-release.sh` still builds against glibc while releases are static musl (deferred to the batch-5 report), and `examples/inbox` demonstrates neither `Counter`/`Gauge` nor a declared client field. Measurements still open in docs/PENDING_MINI_ROUNDS.md: CF-12, fix-3, CF-6(a), CF-10 |
